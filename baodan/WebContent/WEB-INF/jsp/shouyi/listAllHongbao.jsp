...
<body onload="body_onload();">
	...
	<div class="rhead">
		<div class="rpos" >出款总额:<span id="zongJinE" style="color: red">${totalMoney}</span>元</div>
		<div class="clear"></div>
	</div>
	<table class="pn-ltable" width="100%" cellspacing="1" cellpadding="0" border="0">
		...
		<tbody class="pn-ltbody">
			...
			<tr onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
				...
				<td align="center" onclick="chakanDingdan(${item.billNo })">${item.billNo }</td>
				<td align="center" onclick="chakanDianpu(${item.dianpuId})">${item.dianpuId}</td>
				<td align="center">${item.amount}</td>
				<td align="center"><fmt:formatDate value="${item.addTime}" pattern="yyyy-MM-dd"/></td>
				<td align="center">
					<c:if test="${item.statusz=='SENDING'}">发放中</c:if>
					<c:if test="${item.statusz=='SENT'}">已发待领取</c:if>
					<c:if test="${item.statusz=='FAILED'}">发放失败</c:if>
					<c:if test="${item.statusz=='RECEIVED'}">已领取</c:if>
					<c:if test="${item.statusz=='REFUND'}">已退回</c:if>
				</td>
				<td align="center">${item.bufaNum}</td>
				<td align="center"><fmt:formatDate value="${item.bufaTime}" pattern="yyyy-MM-dd"/></td>
			</tr>
		</c:forEach>   
		</tbody>
	</table>

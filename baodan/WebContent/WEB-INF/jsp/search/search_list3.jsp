...
<script type="text/javascript">
	...
	function searchClic() {
		...
		frmSearch.action="search!listAllTwo.action?methodFlag=all";
		frmSearch.submit();
	...
	function openExcel() {
		// t_flag = -1 表示是站上的人用				
		window.open("downLoadExcel!init.action"+getParamOne()+"&t_flag=-1");
	}
	...
	function goBack() {
		...
		self.location.href="search!listCountOne.action";
	}
					...
					success : 	function(jsonString) {
								...
								if (jsonString.success) {
									...
									if (jsonString.message == "排除嫌疑成功") {
										document.getElementById("frmSearchOne").action="search!listAllTwo.action?methodFlag=all";
...
<form id="frmSearchOne" name="frmSearchOne" method="post">
	<display:table name="litPager"  requestURI="search!listAllTwo.action"  cellspacing="1" cellpadding="0" id="someLists" class="pn-ltable" style="width:3000px;">
   ...
   <c:if test="${not empty litPager}">
		 ...
		 <c:param name="pageActionUrl" value="search!listAllTwo.action"/>

...
<div class="body-box">
<div class="rhead">
		<div class="rpos">您的当前位置: 财务管理-->订单查询-->重置订单状态</div>
		<div class="clear"></div>
	</div>

<form id="frmOrder" name="frmOrder" method="post" action="orders!chongzhiOrder.action" >

	<table width="100%" class="pn-ftable" cellpadding="2" cellspacing="1" border="0">
		
		<tr>
			<td width="20%" class="pn-flabel pn-flabel-h">重置到哪一个状态：</td>
		  <td width="80%" class="pn-fcontent">
		  	<input type="hidden" id="ordersId" name="ordersId"  value="${orders.ordersId}" />
		  	<select id="order_status" name="order_status">
		  		<option value="${orders.order_status }">
		  			<c:if test="${orders.order_status == 0}">未支付</c:if>
		  			<c:if test="${orders.order_status == 1}">已支付</c:if>  
		  			<c:if test="${orders.order_status == 2}">异常</c:if>  
		  			<c:if test="${orders.order_status == 3}">已发货</c:if>  
		  			<c:if test="${orders.order_status == 4}">完成</c:if>  
		  			<c:if test="${orders.order_status == 5}">已退货</c:if>  
		  			<c:if test="${orders.order_status == 6}">已收货</c:if>    
		  		</option>

      ...
      <div id="ui-body-test" class="ui-body ui-body-a ui-corner-all">
        ...
        <div class="ui-field-contain">
	     ...
	     <input type="hidden" name="ivmCountry.createDate"  value="${ivmCountry.createDate }"/>
  		...
  		<c:choose>
   			<c:when test="${sessionScope.loginUser.userId==ivmCountry.userId||ivmCountry.id == null}">
   				<form name="demoForm1" id="demoForm1" class="file" method="post" enctype="multipart/form-data">
			        ...
			        <div id="pic" class="ui-field-contain">
			        	<label for="name1"></label>
			            <img id="headFile11" width="400px" height="250px" src="<%=request.getContextPath() %>${ivmCountry.picUrl}" />
			        </div>
		         </form>
   			</c:when>
   			<c:otherwise> 
         ...
         <c:choose>
   			<c:when test="${sessionScope.loginUser.userId==ivmCountry.userId||ivmCountry.id == null}">
   				<form name="demoForm2" id="demoForm2" class="file" method="post" enctype="multipart/form-data">
			        ...
			        <div id="video" class="ui-field-contain">
        ...
        <c:choose>
   				...
   				<form name="demoForm3" id="demoForm3" class="file" method="post" enctype="multipart/form-data">
				 	<div class="ui-field-contain">
			            <label for="name3">语音介绍：</label>
			       		<input type="file" id="headFile3"  name="filedata" onchange="uploadVoiceAjax('demoForm3','headFile3','bankFront_input3')" class="reg_add_btn">	        
			        </div>
			        <div class="ui-field-contain">
   			...
   			<c:otherwise> 
       			<div id="voice" class="ui-field-contain">
		        	<label for="name2">语音介绍：</label>
		            <div id="a2"></div>
		        </div>
   			</c:otherwise>
    	</c:choose>
...
//var video = ['http://101.201.74.48:81/weishangcheng/upload/video/RARBG.com.mp4'];
...
function uploadFileAjax(fromId,inputName,bankFrontdirUrl){
	...
	$(document).ready(function(){
		...
		if(videoUrl == ""){
			...
			$("#video").show();
			 ...
			 var video=[url];
        	 CKobject.embed('<%=request.getContextPath()%>/js/ckplayer/ckplayer.swf','a1','ckplayer_a1','400','250',false,flashvars,video,params);
		}
		//音频
		var voiceUrl = $.trim($("#bankFront_input31").val());
		if(voiceUrl == ""){
			$("#voice").hide();
		}else{
			$("#voice").show();
			 var url = '<%=request.getContextPath()%>'+voiceUrl;
			 var flashvars={
				        f:url,
				        c:0
				};
			 var video=[url];
        	 CKobject.embed('<%=request.getContextPath()%>/js/ckplayer/ckplayer.swf','a2','ckplayer_a2','400','250',false,flashvars,video,params);
		}
		//图片
		var picUrl = $.trim($("#bankFront_input11").val());
		if(picUrl != ""){
			$("#pic").show();
		}else{
			$("#pic").hide();
	...
	function uploadVoiceAjax(fromId,inputName,bankFrontdirUrl){
		  var f=$("#"+inputName).val(); 
		  if(f==""){ 
	    	  alert("请上传音频文件");
	    	  return false;
		  ...
		  $("#"+fromId).ajaxSubmit({
			  	url:"<%=request.getContextPath()%>/file/fileUpload!voiceUpload.action",
			    dataType:"json",
			    success:function(data){
			        if(data.success){
			        	 $("#"+bankFrontdirUrl+"1").val(data.path);

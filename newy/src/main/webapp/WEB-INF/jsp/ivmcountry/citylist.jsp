...
<div id="wrap">
  ...
  <div id="mainer">
    ...
    <a onclick="javascript:check('${hot.cityId}')" class="hot-city"> <c:out value="${hot.shortname }"/></a>
    </c:if>
    </c:forEach>
			...
			<c:forEach var="m" items="${citymap}">
				<div class="list-table">
					...
					<c:if test="${not empty city.shortname}"> 
						...
						<a onclick="javascript:check('${city.cityId}')">
...
<script type="text/javascript">
     ...
     function check(id){
 		...
 		function bdGetPosition(result){
 			...
 			$.ajax({
 				type: "POST",
 				url:"<%=request.getContextPath()%>/city/city5dAction!choiceCity.action",
 				data:{
 					"cityId":id,
 					"cityNames":cityNames
 				},
 				...
 				success:function(data) {
 					if(data.success){
 						var cityId = data.city.cityId;
 						window.location.href = '<%=request.getContextPath()%>/user/ivmcAction!ivmCountryList.action?ivmCountry.cityId='+cityId;
 					}else{
 						alert(data.tipMessage);
 						//var tipMessage = data.tipMessage;
 						window.location.href = '<%=request.getContextPath()%>/user/ivmcAction!ivmCountryList.action';
 					}
 				}
 			});
 		}	

				...
				<ul class="slides">
					...
					<c:if test="${prod.prodId==2}">
						<a href="#" ontouchmove="shows(1)">
							<li><img
								src="<%=request.getContextPath()%>/images/miaosha/mianmo/mm3banner.jpg"
								style="width: 100%"></li>
						</a>
					</c:if>
					<c:if test="${prod.prodId==82}">
						<a href="#" ontouchmove="shows(3)"> <img
							src="<%=request.getContextPath()%>/images/miaosha/jiayongjinghuaqi/banna.jpg"
							style="width: 100%">
						</a>
					</c:if>
				</ul>
		...
		<div class="det-middle">
			...
			<div class="det-pro-num">
						...
						<div class="pro-color">
							...
							<c:forEach var="p" items="${typelist}" varStatus="vstatus">
								<c:if test="${p.prodState ==1 }">
									<a id="xuanze_${p.prodId}" class="xuanze" name="product"
										guige="${p.prodColor}${p.prodSize}" stock="${p.stock}"
										transFee="${p.transFee}" prodId="${p.prodId}"
										price="${p.price}" onclick="checkProd(this)"> <c:out
											value="${p.prodColor}${p.prodSize}" /> <c:if
											test="${p.stock<500 }">
				...
				<br> <input type="checkbox" name="diyong" id="diyong"
					...
					style="color: red;"> 当前星火券：<span id="max_xhq_count"><c:out
							value="${xhq}" /></span>星火券，是否使用 ？
				</label> <span id="diyiong_info" style="display: none;"> <br>请输入星火券：<input
		...
		<div id="miaoshu" class="shangpintu">
					...
					<ul>
						<c:if test="${prod.prodId==2||prod.prodId==3}">
							<li><img style="width: 100%" alt=""
								src="<%=request.getContextPath()%>/images/miaosha/mianmo/6400.jpg"></li>
						</c:if>
					</ul>
				</c:if>
			</div>
			...
			<div id="product-detail-1">
				<c:if test="${prod.prodId==4}">
					<ul>
						<li><img style="width: 100%" alt=""
							src="<%=request.getContextPath()%>/images/miaosha/jinghuaqi/jhq_05.jpg"></li>
					</ul>
				</c:if>
			</div>

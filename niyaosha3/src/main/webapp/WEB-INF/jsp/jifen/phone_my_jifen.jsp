...
function insertcode(){
	...
	$.ajax({
		type: "POST",
		url:"<%=request.getContextPath()%>/jifen/jifenAction!ajaxmyJiFenList.action",
		data:{
			"userId":'${user.userId }',
			"pageNum":pageNum,
			"date1":date1, 
	    ...
	    success: function(data) {
	    	if (data.success==true) {
	    		...
	    		for(var i=0;i<children.length;i++){
	    			...
	    			child+=('<em>'+children[i][4]+'</em></div>');
	    			child+=('<div class="item_m" id="item_m'+children[i][0]+'">');
	    			child+=('<ul><li>积分：<em>'+children[i][3]+'</em></li>');
	    			child+=('<li>来自ID：<em>'+children[i][2]+'</em></li>');
	    			child+=('<li>接收ID：<em>'+children[i][10]+'</em></li>');
	    			if(children[i][7]==1){
	    			child+=('<li>积分状态：<em>获得</em></li>');
	    		    }else if(children[i][7]==2){
	    		    	child+=('<li>积分状态：<em>支出</em></li>');
	    		    }else{
	    		    	child+=('<li>积分状态：<em>无状态</em></li>');
	    		    }
	    			child+=('<li>获得方式：<em>');
	    			if(children[i][6]==1){
	    				child+=('自己订单返利');
	    				...
	    				child+=('他人订单返利');
	    			}else if(children[i][6]==3){
	    				child+=('抽奖获得');
	    			}else if(children[i][6]==4){
	    				child+=('购买兑换');
	    			}else if(children[i][6]==5){
	    				child+=('新用户关注');
	    			}
	    			child+=('</em></li>');
	    			child+=('<li>来自订单：<em>'+children[i][6]+'</em></li>');
	    			child+=('<li>创建时间：<em>'+time+'</em></li></div></div></div>');
...
$(document).ready(function () {
	...
	<div class="wrapper" id="appendUl">
		...
		<%-- <div class="con">
			...
			<form method="post" action="jifenAction!myJiFenList.action?orderNo=0&requestType=mo & status=document.getElementById('status').value & date1=document.getElementById('date1').value & date2=document.getElementById('date2').value">
            ...
            <input type="hidden" class="input" id="status" name="status" value="${status}">
			...
			<!-- 等待收货 -->
			  ...
			  <c:if test="${fn:length(myjifen) > 0}">
			
			<%-- <p class="tj"> 已付款订单  </p>--%>
			  </c:if>
			
			<c:forEach items="${myjifen}" var="jifen" varStatus="status">
			<%-- <p class="tj">  ${ status.index + 1} 级会员  </p> --%>
        <div class="my_order" >
          <div class="item">
            <div class="item_t"><em>${jifen[4] }</em></div>
            <div class="item_m">
                    <ul>
						<li>积分：<em>${jifen[3]}</em></li>
						<li>来自ID：<em>${jifen[2]}</em></li>
						<li>接收ID：<em>${jifen[10]}</em></li>
						<li>积分状态：
						 <em>
						  <c:if test="${jifen[7]==1 }">已获得</c:if>
						  <c:if test="${jifen[7]==2 }">支出</c:if>
						 </em>
						</li>
						 <li>获得方式：
						 <em>
						  <c:if test="${jifen[8]==1 }">自己订单返利</c:if>
						  <c:if test="${jifen[8]==2 }">他人订单返利</c:if>
						  <c:if test="${jifen[8]==3 }">抽奖获得</c:if>
						  <c:if test="${jifen[8]==4 }">购买兑换</c:if>
						  <c:if test="${jifen[8]==5 }">新用户关注</c:if>
						 </em>
					    </li>
						<li>来自订单：<em>${jifen[6] }</em></li>
						<li>创建时间：<em>${jifen[1]}</em></li>

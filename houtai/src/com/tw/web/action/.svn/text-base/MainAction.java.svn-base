...
public class MainAction extends ExtJSONActionSuport {
	...
	public String goToLeft () {
		...
		if (user instanceof User) {
			...
			Double totalJiFen = fhRecordDAO.findAllJiangjing(user1.getUserId());
			ServletActionContext.getRequest().setAttribute("totalJiFen", totalJiFen);
			
			Double totalTJiFen = fhRecordDAO.findTotalJiFenOneByUserId(user1.getUserId(),new String[]{"3","4","5","9","10","11","13","14","15"});
			ServletActionContext.getRequest().setAttribute("totalTJiFen", totalTJiFen);
			System.out.println(totalJiFen);
			ServletActionContext.getRequest().setAttribute("users", user1);
		}
		else if(user instanceof AdminUser) {
			AdminUser user1 = (AdminUser)user;
			user1 = (AdminUser)adminUserDAO.findById(user1.getUserId());
			ServletActionContext.getRequest().setAttribute("users", user1);
		}

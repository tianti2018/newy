...
@Results(
		{
			...
			@Result(name="goBackList22", type=ActionChainResult.class, 	value="card", params = {"method", "obtainCard"}),
...
public class CardAction extends ExtJSONActionSuport {
	...
	public String canyuhuodong() {
		...
		/*//注销卡
		Card card = cardDAO.findCardByCardNo(cardNo,cardPassword);
		if (null !=card) {
			card.setCardStatus("1"); //已经使用过了
			cardDAO.update(card);
		}
		else {
			ServletActionContext.getRequest().setAttribute("message", "您的卡号不存在，或者已经使用过了，请找管理员进行核实谢谢");
			return "";
		}*/
		
		Double totalBaoDanbi = fhRecordDAO.findTotalJiFenByUserId(loginUser.getUserId()); //该用户的所有积分
		
		if (null!=totalBaoDanbi&&totalBaoDanbi<300) {
			ServletActionContext.getRequest().setAttribute("message","您的积分不够参与活动谢谢");
			return "intcanyuhuodong";
		}
		
		/*//给用户一个序号
		BigInteger maxOrder = userDAO.findFinalOrder();
		...
		BigInteger parentUoId = BigInteger.valueOf(CommUtils.parentId(maxOrder.intValue()));
		userOrder.setParentUoId(parentUoId);
		...
		*/
		//即时结算给积分
		//fhuserOrder(parentUoId,1,map,map1);
	...
	public void fhuserOrder(BigInteger parentUoId,int flag,Map map,Map map1) {
		if (flag<11) {
			...
			if (map1.containsKey(userOrder.getOrderOne().intValue())) { //公司
				if (totalJifen>=292968750) {
					return;
				}
			}
			else if (map.containsKey(userOrder.getOrderOne().intValue())) {  //代理商
				if (totalJifen>=1000000) {
					return;
				}
			}
			else {
				if (totalJifen>=468750) { //普通会员
					return;
				}
			}
			...
			if (null!=userOrder) {
				...
				fhRecord.setCreateDate(new Date());
				fhRecord.setFhType("3"); // 公司给会员30积分
				fhRecord.setUoId(userOrder.getUoId());
				
				fhRecordDAO.create(fhRecord);
				BigInteger parentUoIdOne = BigInteger.valueOf(CommUtils.parentId(parentUoId.intValue()));
				flag++;
				fhuserOrder(parentUoIdOne,flag,map,map1);
			}
		}
	}
	...
	public String initCard() {
		...
		User loginUser = (User)ServletActionContext.getRequest().getSession().getAttribute("user");
		if(loginUser!=null){
			List list=ordersDAO.findEntityByPropertiName("userId",loginUser.getUserId());
	...
	public String changeCard () throws Exception { //跟新卡的状态
		Card card = (Card)cardDAO.findById(cardId);
		User toUser = userDAO.findUserByLoginName(otherLoginName);
		User loginUser = (User)ServletActionContext.getRequest().getSession().getAttribute("user");
		if (null==toUser) {
			ServletActionContext.getRequest().setAttribute("message", "此用户不存在");
			return "goBackList22";
		}
		else if (toUser.getUserId().equals(loginUser.getUserId())) {
			ServletActionContext.getRequest().setAttribute("message", "自己不能给自己转");
			return "goBackList22";
		}
		
		if (null !=card && "1".equals(card.getCardStatus())) {
			ServletActionContext.getRequest().setAttribute("message", "此卡已经使用过了！！！");
			return "goBackList22";
		}
		
		card.setUserId(toUser.getUserId());
		card.setFormUserId(loginUser.getUserId());
		card.setCreateDate(new Date());
		cardDAO.update(card);
		
		ServletActionContext.getRequest().setAttribute("message", "转换成功");
		return "goBackList22";
	}

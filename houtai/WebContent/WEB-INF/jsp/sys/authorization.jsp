...
<script language="javascript" type="text/javascript">
	...
	function changeFunction() {
		...
		self.location.href="roleFunction!listAll.action?roleId="+roleId;			
	} 		
</script>
...
<div class="body-box">
<div class="rhead">
	<div class="rpos">您的当前位置-->角色授权</div>
	<div class="clear"></div>
</div>
<div class="rhead">
	<form  method="post">
...
<form id="frmAuth"  name="frmAuth" method="post" action="roleFunction!create.action" >
...
<tbody class="pn-ltbody">
 ...
 <s:iterator value="litPager" var="item"  status="status">
 
<tr onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
	<td><s:property value="#status.index+1"/></td>
	<td>
		<s:if test='%{module=="1"}'>后台管理</s:if>
		<s:elseif test='%{module=="2"}'>信息管理</s:elseif>
		<s:elseif test='%{module=="3"}'>系统管理</s:elseif>
		<s:elseif test='%{module=="4"}'>财务管理</s:elseif>
	</td>	
	<td><s:property value="functionName"/></td>
		
	<td>
		<s:if test="functionId in #request.had ">
			<s:checkbox theme="simple" name="functionId" fieldValue="%{functionId}" value="true">
			
			</s:checkbox>
			<%-- <input type="checkbox" id="functionId${status.index+1}" name="functionId" value="${item.functionId}"/> --%>
		</s:if>	
		<s:else>
			<s:checkbox theme="simple" name="functionId" fieldValue="%{functionId}" value="false">			
			</s:checkbox>
		</s:else>							
	</td>
